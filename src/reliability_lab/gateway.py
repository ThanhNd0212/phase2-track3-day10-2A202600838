...
class ReliabilityGateway:
    ...
    def complete(self, prompt: str) -> GatewayResponse:
        ...
        last_error: str = ""
        cheapest_cost = min(p.cost_per_1k_tokens for p in self.providers)
        for i, provider in enumerate(self.providers):
            # Budget at 100%: serve from cache only or static fallback
            if self._cumulative_cost >= self.cost_budget:
                break
            # Budget at 80%: skip expensive providers, route to cheapest only
            if (self._cumulative_cost >= self.cost_budget * 0.8
